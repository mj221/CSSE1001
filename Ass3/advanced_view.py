...
from tower import AbstractTower, TurretTower, MissileTower, PulseTower, SimpleTower, \
    AbstractObstacle, Missile, Pulse, Turret
...
##        (CoinTower, '_draw_coin'),
        (TurretTower, '_draw_turret')
    ])
    ...
    @classmethod
    def _draw_turret(cls, canvas: tk.Canvas, tower_: TurretTower):
        """Draws a missile tower"""
        ...
        for delta_angle in (-math.pi/12, math.pi/12):
            tags.append(
                canvas.create_line(x, y, x + (x_diameter / 4) * cell_size * math.cos(angle + delta_angle),
                                   y + (y_diameter / 4) * cell_size * math.sin(angle + delta_angle),
                                   tag='tower')
            )
...
##    @classmethod
##    def _draw_coin(cls, canvas: tk.Canvas, tower_: AbstractTower):
##        x, y = tower_.position
##
##        x_diameter, y_diameter = tower_.grid_size
##        top_left, bottom_right = tower_.get_bounding_box()
...
##            dy = (y_diameter/2) * cell_size * math.sin(angle)
##
##            tag = canvas.create_line(x,y, x + dx/3, y + dy/3,  tag='tower')
##
##            tags.append(tag)
##
##
...
class ObstacleView(SimpleView):
    ...
    @classmethod
    def _draw_turret(cls, canvas: tk.Canvas, turret: Turret):
        """Draws a missile"""

        x, y = turret.position

        length, width = turret.size

        dx, dy = rotate_point((length / 1, width / 1), turret.rotation)

        head = x + dx, y + dy
        tail = x - dx, y - dy

        return canvas.create_line(head, tail, fill = turret.colour, tag='obstacle')
