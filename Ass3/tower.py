...
class Turret(AbstractObstacle):
    """A simple projectile fired from a Turret"""
    name = "Turret"
    colour = 'red'
    ...
    def step(self, units):
        ...
        if radius <= self.speed:
            self.target.damage(self.damage, 'turret')
            return False, None
...
class TurretTower(SimpleTower):
    ...
    def __init__(self, cell_size: int, grid_size=(.9, .9), rotation=math.pi * .25, base_damage=25, level: int = 1):
        super().__init__(cell_size, grid_size=grid_size, rotation=rotation, base_damage=base_damage, level=level)
    ...
    def step(self, units):
        ...
        # Spawn missile on tower
        turret = Turret(self.position, self.cell_size, target, rotation=self.rotation,
